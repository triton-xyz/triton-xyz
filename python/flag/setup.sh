...
pushd "$ROOT/third_party/triton-flaggems" >/dev/null
...
rm -f "$ROOT/third_party/triton-flaggems/tests/conftest.py"
ln -s "$ROOT/python/flag/conftest.py" \
  "$ROOT/third_party/triton-flaggems/tests/conftest.py"

rm -f "$ROOT/python/flag/tests"
ln -s "$ROOT/third_party/triton-flaggems/tests" \
