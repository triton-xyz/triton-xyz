...
// CHECK:           %[[SPLAT_0:.*]] = tt.splat %[[ARG2]] : i32 -> tensor<4xi32>
// CHECK:           %[[CMPI_0:.*]] = arith.cmpi slt, %[[MAKE_RANGE_0]], %[[SPLAT_0]] : tensor<4xi32>
// CHECK:           %[[GATHER_0:.*]] = tts.gather %[[ARG0]]{{\[}}%[[MAKE_RANGE_0]]] mask = %[[CMPI_0]] default = %[[CONSTANT_0]] : (<f32>, tensor<4xi32>) -> tensor<4xf32>
// CHECK:           tts.scatter %[[GATHER_0]] into %[[ARG1]]{{\[}}%[[MAKE_RANGE_0]]] mask = %[[CMPI_0]] : tensor<4xf32> into (<f32>, tensor<4xi32>)
// CHECK:           tt.return
// CHECK:         }
...
// CHECK-SAME:      %[[ARG0:[0-9]+|[a-zA-Z$._-][a-zA-Z0-9$._-]*]]: !tt.ptr<f32>,
// CHECK-SAME:      %[[ARG1:[0-9]+|[a-zA-Z$._-][a-zA-Z0-9$._-]*]]: !tt.ptr<f32>) {
// CHECK:           %[[CONSTANT_0:.*]] = arith.constant dense<5> : tensor<4xi64>
// CHECK:           %[[MAKE_RANGE_0:.*]] = tt.make_range {end = 4 : i32, start = 0 : i32} : tensor<4xi32>
// CHECK:           %[[EXTSI_0:.*]] = arith.extsi %[[MAKE_RANGE_0]] : tensor<4xi32> to tensor<4xi64>
// CHECK:           %[[ADDI_0:.*]] = arith.addi %[[EXTSI_0]], %[[CONSTANT_0]] : tensor<4xi64>
// CHECK:           %[[GATHER_0:.*]] = tts.gather %[[ARG0]]{{\[}}%[[ADDI_0]]] : (<f32>, tensor<4xi64>) -> tensor<4xf32>
// CHECK:           tts.scatter %[[GATHER_0]] into %[[ARG1]]{{\[}}%[[MAKE_RANGE_0]]] : tensor<4xf32> into (<f32>, tensor<4xi32>)
// CHECK:           tt.return
// CHECK:         }
...
// CHECK:           %[[MAKE_RANGE_0:.*]] = tt.make_range {end = 4 : i32, start = 0 : i32} : tensor<4xi32>
// CHECK:           %[[FOR_0:.*]]:2 = scf.for %[[VAL_0:.*]] = %[[CONSTANT_0]] to %[[ARG2]] step %[[CONSTANT_1]] iter_args(%[[VAL_1:.*]] = %[[MAKE_RANGE_0]], %[[VAL_2:.*]] = %[[MAKE_RANGE_0]]) -> (tensor<4xi32>, tensor<4xi32>)  : i32 {
...
// CHECK:             %[[ADDI_0:.*]] = arith.addi %[[VAL_1]], %[[MAKE_RANGE_0]] : tensor<4xi32>
// CHECK:             %[[ADDI_1:.*]] = arith.addi %[[VAL_2]], %[[MAKE_RANGE_0]] : tensor<4xi32>
// CHECK:             scf.yield %[[ADDI_0]], %[[ADDI_1]] : tensor<4xi32>, tensor<4xi32>
// CHECK:           }
// CHECK:           tt.return
// CHECK:         }
...
// CHECK:           %[[MULI_0:.*]] = arith.muli %[[GET_PROGRAM_ID_0]], %[[CONSTANT_3]] : i32
// CHECK:           %[[ADDI_0:.*]] = arith.addi %[[MULI_0]], %[[CONSTANT_0]] : i32
// CHECK:           %[[MAKE_TENSOR_PTR_0:.*]] = tt.make_tensor_ptr %[[ARG0]], {{\[}}%[[CONSTANT_2]]], {{\[}}%[[CONSTANT_1]]], {{\[}}%[[ADDI_0]]] {order = array<i32: 0>} : <tensor<4xf16>>
// CHECK:           %[[LOAD_0:.*]] = tt.load %[[MAKE_TENSOR_PTR_0]] : !tt.ptr<tensor<4xf16>>
// CHECK:           %[[MAKE_TENSOR_PTR_1:.*]] = tt.make_tensor_ptr %[[ARG1]], {{\[}}%[[CONSTANT_2]]], {{\[}}%[[CONSTANT_1]]], {{\[}}%[[CONSTANT_4]]] {order = array<i32: 0>} : <tensor<4xf16>>
