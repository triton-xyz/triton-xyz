...
// CHECK:           %[[MAKE_GATHER_SCATTER_TPTR_0:.*]] = tts.make_gather_scatter_tptr %[[ARG0]] to sizes: [4, 4] gather_scatter_dim: 0 gather_scatter_offset: %[[CONSTANT_0]], strides: [4, 1], offsets: [0, 0] : tensor<4xi32>  <f32> to tensor<4x4x!tt.ptr<f32>>
// CHECK:           %[[LOAD_0:.*]] = tt.load %[[MAKE_GATHER_SCATTER_TPTR_0]] {tta.fallback, tta.fallback_reason = "ptr_expr_analysis_failed"} : tensor<4x4x!tt.ptr<f32>>
...
// CHECK:         }
  tt.func @fallback_gather_scatter_tts_ptr(%base: !tt.ptr<f32>, %dst: !tt.ptr<f32>) {
    ...
    %src = tts.make_gather_scatter_tptr %base to sizes: [4, 4] gather_scatter_dim: 0 gather_scatter_offset: %offsets, strides: [4, 1], offsets: [0, 0] : tensor<4xi32> <f32> to tensor<4x4x!tt.ptr<f32>>
    %dst_tptr = tts.make_tptr %dst to sizes: [4, 4], strides: [4, 1], offsets: [0, 0], shape: [0, 0], order: [] : <f32> to tensor<4x4x!tt.ptr<f32>>
    %val = tt.load %src : tensor<4x4x!tt.ptr<f32>>
    tt.store %dst_tptr, %val : tensor<4x4x!tt.ptr<f32>>
    tt.return
  }
}
