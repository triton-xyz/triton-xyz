...
bool isCpuInstrumentationSession(const std::string &profilerName,
                                 const std::string &mode) {
  if (toLower(profilerName) != "instrumentation") {
    return false;
  }
  auto modeAndOptions = proton::split(mode, ":");
  return !modeAndOptions.empty() && toLower(modeAndOptions.front()) == "cpu";
}
...
std::unique_ptr<Data> makeData(const std::string &dataName,
                               ...
                               const std::string &mode) {
  const auto normalizedDataName = toLower(dataName);
  if (isCpuInstrumentationSession(profilerName, mode)) {
    if (normalizedDataName == "tree") {
      return std::make_unique<CpuInstrumentationTreeData>(path, contextSource);
    } else if (normalizedDataName == "trace") {
      return std::make_unique<CpuInstrumentationTraceData>(path, contextSource);
    }
  }
