...
def check_dtype(fill_value, dtype, device):
    if isinstance(fill_value, bool):
    ...
    elif (dtype in ALL_INT_DTYPES and (fill_value < torch.iinfo(dtype).min or fill_value > torch.iinfo(dtype).max)) or (
        ...
        raise RuntimeError(f"value cannot be converted to type {dtype} without overflow")
    if dtype == torch.float64:
        fill_value = torch.tensor(fill_value, dtype=dtype, device=device)
    return fill_value


def full_(out, n_elements, fill_value):
...
def full(size, fill_value, *, dtype=None, layout=None, device=None, pin_memory=None):
    ...
    return full_(out, n_elements, fill_value)
